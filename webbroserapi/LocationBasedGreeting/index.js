...
function displayGreeting(city) {
    var greetingEl = document.getElementById("greeting");
    if (!greetingEl) {
        return;
    }
    else {
        greetingEl.textContent = "Hello from ".concat(city);
    }
}
// function to fetch city name using coordinates
function getCityName(latitude, longitude) {
    return __awaiter(this, void 0, void 0, function () {
        var apiKey, url, response, data, cityName, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    apiKey = "66cb4c9157b85b76f0b7a7d81efa3b1f";
                    url = "https://api.openweathermap.org/data/2.5/weather?lat=".concat(latitude, "&lon=").concat(longitude, "&appid=").concat(apiKey);
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                ...
                case 2:
                    response = _a.sent();
                    if (!response.ok)
                        throw new Error("Failed to fetch the name of the city ");
                ...
                case 4:
                    error_1 = _a.sent();
                    console.error("Error fetching  city name", error_1.message);
                    return [3 /*break*/, 5];
